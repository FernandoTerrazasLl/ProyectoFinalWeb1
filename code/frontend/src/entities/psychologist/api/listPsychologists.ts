...
export function listPsychologists(query: PsychologistQuery = {}): Promise<Result<Psychologist[], HttpError>> {
  ...
  return http.request<Psychologist[]>("GET", `/psychologists${search ? `?${search}` : ""}`);
}
